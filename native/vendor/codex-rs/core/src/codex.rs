...
impl SessionConfiguration {
    ...
    pub(crate) fn for_embedded_host(
        config: Arc<Config>,
        collaboration_mode: CollaborationMode,
        base_instructions: String,
        session_source: SessionSource,
        ...
        Self {
            ...
            file_system_sandbox_policy: config.permissions.file_system_sandbox_policy.clone(),
            network_sandbox_policy: config.permissions.network_sandbox_policy,
            windows_sandbox_level: WindowsSandboxLevel::from_config(config.as_ref()),
            cwd: config.cwd.clone(),
            codex_home: config.codex_home.clone(),
            thread_name: None,
...
impl Session {
    ...
    /**
     * Keep approval state available even when the session is only used for
     * local tool execution and no model task is running.
     */
    pub(crate) async fn ensure_active_turn_state(&self) {
    ...
    /**
     * Build a detached turn context from the current session settings.
     * This is used by local tool hosts that do not run the normal turn loop.
     */
    pub(crate) async fn create_detached_turn_context(
        self: &Arc<Self>,
        sub_id: String,
    ) -> Arc<TurnContext> {
        let session_configuration = {
        ...
        let plugin_outcome = self
            ...
            .await;
        let effective_skill_roots = plugin_outcome.effective_skill_roots();
        let skills_input = crate::skills_load_input_from_config(&per_turn_config, effective_skill_roots);
        let fs = self
            .services
            .environment
            .as_ref()
